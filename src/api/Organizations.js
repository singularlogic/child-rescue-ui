...
export default {
    others: () => secured.get(`${resource}/others/`),
    all: () => secured.get(`${resource}/`),
    get: id => secured.get(`${resource}/${id}/`),
    edit: payload => secured.patch(`${resource}/${payload.id}/`, payload),
    updateImage: (id, payload, headers) =>
        secured.put(`${resource}/${id}/`, payload, headers),

    getUsers: organizationId =>
        secured.get(`${resource}/${organizationId}/users/`),
    getUser: (organizationId, params) =>
        secured.get(`${resource}/${organizationId}/user/`, { params }),
    updateUser: (organizationId, payload, params) =>
        secured.put(`${resource}/${organizationId}/user/`, payload, { params }),
