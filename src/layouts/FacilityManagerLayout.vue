        ...
        <v-toolbar :clipped-left="false" flat :scroll-threshold="1" :scroll-off-screen="false" app class="toolbar" tabs>
            ...
            <template v-slot:extension v-if="$route.meta.toolbar && caseObject.status!=='archived' && caseObject.status!==null && caseObject.status!==undefined">
                <v-tabs v-model="tab" color="transparent" align-with-title class="ml-0 pl-0">
                    ...
                    <v-tab :ripple="false" :to="`/cases/${$route.params.id}/files`">{{ $t('layouts.files') }}</v-tab>
                    ...
                    <v-tab v-if="caseObject.organization===$store.state.organizationId && caseObject.organization===$store.state.organizationId" :ripple="false" :to="`/cases/${$route.params.id}/analytics`">{{ $t('layouts.analytics') }}</v-tab>
                </v-tabs>
            </template>
        </v-toolbar>
        ...
        this.drawerItems.push(
            {
                ...
                isEnabled: true,
                icon: 'list',
                text: this.$t('layouts.manage_facility'),
                routerLink: '/cases',
            },
        );
        await this.loadData();
